...
const main = async () => {
  ...
  const bufferSize =
    textureSize[0] * textureSize[1] * 4 * Float32Array.BYTES_PER_ELEMENT;
  ...
  computePass.dispatchWorkgroups(
    Math.ceil(textureSize[0] / workgroup[0]),
    Math.ceil(textureSize[1] / workgroup[1])
  );
  computePass.end();
